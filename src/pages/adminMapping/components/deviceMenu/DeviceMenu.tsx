...
import deviceOptionData from '../../data/deviceFilterOptionData'

const DeviceMenu = observer(() => {
  return (
    <>
      <h2 className="mb-2 text-2xl font-semibold text-black">{deviceOptionData[0].title}</h2>
      <div className="mb-8 flex w-full flex-col rounded border border-textDisabled bg-bgPaper py-4 px-12">
        <div className="mb-4 flex flex-wrap justify-center gap-4">
          {deviceOptionData.map((data: any, idx: number) => (
            <div key={idx}>
              <h3 className="mb-1 block text-sm font-semibold text-gray-900 dark:text-gray-400">
                {data.name}
              </h3>
              <select className="block w-40 rounded border border-gray-300 bg-gray-50 p-2.5 text-sm text-gray-900 focus:border-blue-500 focus:ring-blue-500">
